# Hi there, I'm Shuvat Bukinski 👋

### Software Developer | B.Sc. Computer Science Student @ Bar-Ilan University

I'm passionate about **backend development** and **agentic AI systems** — I love building intelligent automation and efficient technical solutions that solve real-world problems. Experienced in real-time operational analysis and problem-solving under pressure.

🎯 **Currently looking for:** a role focused on developing agentic AI systems and intelligent automation.

📫 **Connect with me:**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/shuvat-bukinski-a07510264)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:shuvatbuk@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/shuvat)

---

## 🛠️ Tech Stack & Tools

**Languages:**

[![My Languages](https://skillicons.dev/icons?i=java,c,cpp,py,js,ts)](https://skillicons.dev)

**Frontend & Backend:**

[![My Skills](https://skillicons.dev/icons?i=react,nodejs,express,mongodb,postgres,html,css)](https://skillicons.dev)

![React Native](https://img.shields.io/badge/React_Native-61DAFB?style=flat-square&logo=react&logoColor=black)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat-square&logo=streamlit&logoColor=white)

**AI & Agentic Development:**

![LangGraph](https://img.shields.io/badge/LangGraph-1C3C3C?style=flat-square&logo=langchain&logoColor=white)
![Agentic Workflows](https://img.shields.io/badge/Agentic_Workflows-5E35B1?style=flat-square)
![Multi--Agent Architecture](https://img.shields.io/badge/Multi--Agent_Architecture-3949AB?style=flat-square)

**Tools & Platforms:**

[![Tools](https://skillicons.dev/icons?i=git,docker,linux,bash,powershell)](https://skillicons.dev)

![Jira](https://img.shields.io/badge/Jira-0052CC?style=flat-square&logo=jira&logoColor=white)
---

## 📊 GitHub Statistics

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=shuvat&show_icons=true&theme=tokyonight&hide_border=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=shuvat&layout=compact&theme=tokyonight&hide_border=true)

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=shuvat&theme=tokyonight&hide_border=true)

[![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=shuvat&theme=tokyo-night&hide_border=true)](https://github.com/shuvat)

---

## 📌 Featured Projects

### 🗄️ Full-Stack Storage Application — MERN & C++

* **Tech Stack:** React, Node.js, MongoDB, C++, TCP Sockets
* **Description:** Scalable storage platform with RESTful API architecture and secure JWT authentication.
* **Highlights:** High-efficiency C++ persistence engine for file hierarchies built with TDD, integrated via TCP Sockets.

### 🤖 Auto-Installer for Beginners (AIFB) — Agentic AI System

* **Tech Stack:** Python, LangGraph, Streamlit
* **Description:** Multi-agent AI system that plans and executes Windows software installs from natural-language requests, with a clear separation between planning and execution.
* **Highlights:** Sandboxed desktop agent running only human-approved commands over IPC — keeping a safe boundary between reasoning and system access.

---

## 🎓 Education & Experience

* **B.Sc. Computer Science** — Bar-Ilan University (Oct 2024 – Present, expected graduation March 2028)
  * Relevant coursework: Computer Architecture (92), Advanced Computer Programming (93)
* **Operational Data Analyst** — Israel Border Police: real-time data analysis in a high-pressure tactical environment, awarded a **certificate of excellence** for outstanding performance.

---

## 💡 Interests

* 🤖 Agentic AI & Intelligent Automation
* ⚙️ Backend Development & System Design
* 📚 Reading fiction
* ✈️ Traveling
* 🏋️ Working out

---

### ⚡ What I'm Up To

✨ Building agentic AI systems and intelligent automation
📚 Continuously learning new technologies and best practices
🤝 Open to collaboration and exciting opportunities
